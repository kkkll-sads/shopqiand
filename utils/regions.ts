 ...
 * 中国省市区数据 (包含主要城市的区县数据)
 */
export const REGION_DATA: Record<string, ProvinceData> = {
    '北京市': {
        cities: ['北京市'],
        districts: {
    ...
    '浙江省': {
        ...
        districts: {
            ...
            '宁波市': ['海曙区', '江北区', '北仑区', '镇海区', '鄞州区', '奉化区', '慈溪市', '余姚市']
        }
    },
    '安徽省': {
...
export const getCities = (province: string): string[] => {
    return REGION_DATA[province]?.cities || [];
};
...
export const getDistricts = (province: string, city: string): string[] => {
    const provinceData = REGION_DATA[province];
    if (!provinceData?.districts) return [];
    return provinceData.districts[city] || [];
};
