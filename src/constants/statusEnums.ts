...
export enum BizType {
  ...
  SIGN_IN = 'sign_in',                           // 签到
  SIGN_IN_REFERRAL = 'sign_in_referral',         // 直推签到奖励
  OLD_ASSETS_UNLOCK = 'old_assets_unlock',       // 老资产解锁
  SCORE_EXCHANGE_GREEN_POWER = 'score_exchange_green_power', // 消费金兑换绿色算力
  FIRST_TRADE_REWARD = 'first_trade_reward',     // 首次交易奖励
  QUESTIONNAIRE_REWARD = 'questionnaire_reward', // 问卷奖励
  AGENT_COMMISSION = 'agent_commission',         // 代理分红
  AGENT_DIRECT_COMMISSION = 'agent_direct_commission', // 直推分红 (代理)
  AGENT_TEAM_COMMISSION = 'agent_team_commission', // 团队分红 (代理)
  RIGHTS_DECLARATION_REWARD = 'rights_declaration_reward', // 确权申报奖励
}
...
export function getRechargeOrderStatusConfig(status: number): OrderStatusUIConfig {
  const configs: Record<number, OrderStatusUIConfig> = {
    0: { text: '待审核', color: 'text-orange-600', bgColor: 'bg-gradient-to-r from-orange-400 to-orange-500', iconName: 'clock' },
    1: { text: '已通过', color: 'text-green-600', bgColor: 'bg-gradient-to-r from-green-400 to-green-500', iconName: 'check-circle' },
    2: { text: '已拒绝', color: 'text-red-600', bgColor: 'bg-gradient-to-r from-red-400 to-red-500', iconName: 'x-circle' },
  };
...
export function getWithdrawOrderStatusConfig(status: number): OrderStatusUIConfig {
  const configs: Record<number, OrderStatusUIConfig> = {
    ...
    1: { text: '已通过', color: 'text-green-600', bgColor: 'bg-gradient-to-r from-green-400 to-green-500', iconName: 'check-circle' },
    2: { text: '已拒绝', color: 'text-red-600', bgColor: 'bg-gradient-to-r from-red-400 to-red-500', iconName: 'x-circle' },
    3: { text: '已打款', color: 'text-blue-600', bgColor: 'bg-gradient-to-r from-blue-400 to-blue-500', iconName: 'check-circle' },
    4: { text: '打款失败', color: 'text-gray-600', bgColor: 'bg-gradient-to-r from-gray-400 to-gray-500', iconName: 'alert-triangle' },
  };
  return configs[status] ?? configs[0];
}
...
export function getConsignmentStatusUIConfig(consignmentStatus: number): {
  ...
  } else if (consignmentStatus === ConsignmentStatus.SELLING) {
