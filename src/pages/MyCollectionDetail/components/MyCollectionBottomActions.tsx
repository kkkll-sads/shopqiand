...
export function MyCollectionBottomActions({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[60] border-t border-gray-100 bg-white/90 p-4 pb-safe backdrop-blur-md">
      <div className="mb-3 flex items-center justify-between px-1">
        <span className="text-sm font-medium text-gray-500">当前估值</span>
        <div className="text-right">
          <span className="font-mono text-lg font-bold text-gray-700">¥{currentValuation}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        <button
          ...
          className={`flex flex-1 items-center justify-center gap-2 rounded-lg py-3.5 font-bold text-white shadow-lg transition-colors active:scale-[0.98] ${
            nodeLoading
              ...
              : 'bg-gray-500 shadow-gray-500/20 active:bg-gray-600'
          }`}
        >
        ...
        <button
          ...
          className={`flex flex-1 items-center justify-center gap-2 rounded-lg py-3.5 font-bold shadow-lg transition-colors active:scale-[0.98] ${
            consignmentLoading
              ? 'cursor-not-allowed bg-[#b97777] text-amber-50 shadow-red-900/10'
              : 'bg-[#8B0000] text-amber-100 shadow-red-900/20 active:bg-[#A00000]'
          }`}
        >
