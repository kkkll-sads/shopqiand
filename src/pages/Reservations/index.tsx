...
export const ReservationsPage = () => {
  ...
  const {
    ...
    async (signal) => {
      const res = await reservationApi.getList({ page: 1, limit: PAGE_SIZE }, signal);
      const list = res?.list ?? [];
      setItems(list);
      setPage(1);
    ...
    { cacheKey: 'reservations:first-page' },
  );
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const res = await reservationApi.getList({ page: nextPage, limit: PAGE_SIZE });
      const list = res?.list ?? [];
