...
const parseResponse = async (response: Response) => {
    ...
    if (contentType && contentType.includes('application/json')) {
        ...
        warnLog('networking', 'API 返回非 JSON 响应', text);
        try {
    ...
    try {
        ...
        if (data.code === 303) {
            ...
            if (disableNeedLoginHandler) {
                warnLog('networking', '公开页面遇到登录错误，抛出普通错误让上层处理');
                throw new Error('公开页面访问失败：' + (data.msg || '请稍后重试'));
            }

            warnLog('networking', '用户登录已失效（code 303），抛出 NeedLoginError 交由上层统一处理');
        ...
        errorLog('networking', `API 调用失败 [${method}] ${url}`, error);
