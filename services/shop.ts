...
export interface CreateOrderParams {
    ...
    pay_type: 'money' | 'score' | 'mixed';
    // 支付金额字段（后端期望 pay_money / pay_score）
    pay_money?: number;
    pay_score?: number;
    // 兼容字段（以前的命名）
    money_amount?: number;
...
export async function createOrder(params: CreateOrderParams): Promise<ApiResponse> {
    ...
    // 支持组合支付字段（兼容多种命名）
    if (params.pay_type === 'mixed') {
        const money = params.pay_money ?? params.money_amount;
        const score = params.pay_score ?? params.score_amount;
        if (money !== undefined) requestBody.pay_money = money;
        if (score !== undefined) requestBody.pay_score = score;
    }
