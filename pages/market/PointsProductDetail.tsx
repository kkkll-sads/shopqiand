...
const PointsProductDetail: React.FC<PointsProductDetailProps> = ({ product, onBack, onNavigate }) => {
    ...
    const handleSubmitOrder = async () => {
        ...
        try {
            ...
            // 若后端提示需要组合支付，则自动尝试用组合参数重试一次
            if (!isSuccess(response)) {
                const msg = String(response?.message || response?.msg || '');
                if (/需要同时支付.*余额.*消费金|需要同时支付余额和消费金/.test(msg)) {
                    // 计算组合支付金额（基于商品/详情数据）
                    const unitCash = Number(detailData?.price ?? product.price ?? 0);
                    const unitScore = Number(detailData?.score_price ?? product.score_price ?? 0);
                    const payMoney = Number((unitCash * quantity).toFixed(2));
                    const payScore = Number(unitScore * quantity);
        ...
        <div className="min-h-screen bg-gray-50 pb-24 relative">
            ...
            <div className="bg-white p-4 mb-2">
                <div className="flex items-baseline mb-2">
                    {Number(detailData?.price) > 0 && Number(detailData?.score_price) > 0 ? (
                        <>
                            <span className="text-base text-orange-600 font-medium">¥{detailData.price}</span>
                            <span className="text-base text-orange-600 font-medium mx-1">+</span>
                            <span className="text-base text-orange-600 font-mono font-bold">{detailData.score_price}</span>
                            <span className="text-base text-orange-600 font-medium">消费金</span>
                        </>
                    ) : isScoreProduct ? (
                        <>
                            <span className="text-base text-orange-600 font-mono font-bold">{detailData?.score_price ?? displayPrice}</span>
                            <span className="text-base text-orange-600 font-medium">消费金</span>
                            {detailData?.price && (
                        ...
                        <>
                            <span className="text-base text-orange-600 font-medium">¥{detailData?.price ?? displayPrice}</span>
                            <span className="text-base text-orange-600 font-medium ml-1">元</span>
                        </>
                    )}
                </div>
                        ...
                        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                                ...
                                <div className="flex-1 flex flex-col justify-between py-1">
                                    ...
                                    <div className="flex justify-between items-end mt-2">
                                        <div className="font-mono font-bold text-orange-600 text-lg">
                                            {Number(detailData?.price) > 0 && Number(detailData?.score_price) > 0 ? (
                                                <>
                                                    <span className="text-base text-orange-600 font-medium">¥{Number(detailData.price).toFixed(2)}</span>
                                                    <span className="text-base text-orange-600 font-medium mx-1">+</span>
                                                    <span className="text-base text-orange-600 font-mono font-bold">{Number(detailData.score_price)}</span>
                                                    <span className="text-base text-orange-600 font-medium ml-1">消费金</span>
                                                    <span className="text-xs text-gray-500 font-normal ml-2">x {quantity}</span>
                                                </>
                                            ) : isScoreProduct ? (
                                                <>
                                                    <span className="text-base text-orange-600 font-mono font-bold">{detailData?.score_price ?? displayPrice}</span>
                            ...
                            <div className="mt-3 flex justify-end items-center gap-2">
                                ...
                                <span className="text-lg font-bold text-orange-600 font-mono">
                                    {Number(detailData?.price) > 0 && Number(detailData?.score_price) > 0 ? (
                                        <>
                                            <span className="text-base text-orange-600 font-medium">¥{(Number(detailData.price) * quantity).toFixed(2)}</span>
                                            <span className="text-base text-orange-600 font-medium mx-1">+</span>
                                            <span className="text-base text-orange-600 font-mono font-bold">{Number(detailData.score_price) * quantity}</span>
                                            <span className="text-base text-orange-600 font-medium ml-1">消费金</span>
                                        </>
