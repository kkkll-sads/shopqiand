...
const PointOrderCard: React.FC<PointOrderCardProps> = ({ 
  ...
  // 订单级支付汇总（用于 mixed 展示）
  const orderPayMoney = Number((order as any).pay_money ?? order.total_amount ?? 0);
  const orderPayScore = Number((order as any).pay_score ?? order.total_score ?? 0);
  const sumPrice = items.reduce((s, it) => s + (Number(it.price) || 0) * (Number(it.quantity) || 1), 0);
  const sumScorePrice = items.reduce((s, it) => s + (Number(it.score_price) || 0) * (Number(it.quantity) || 1), 0);
            ...
            <div key={item.id} className="px-4 pb-4">
              ...
              <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-baseline gap-1">
                    {order.pay_type === 'mixed' && orderPayMoney > 0 && orderPayScore > 0 ? (() => {
                      // 计算 item 的现金/消费金份额（按目录价和消耗价比例分配）
                      const qty = Number(item.quantity) || 1;
                      const itemPriceTotal = (Number(item.price) || 0) * qty;
                      const itemScoreTotal = (Number(item.score_price) || 0) * qty;
                      const cashShare = sumPrice > 0
                        ? Math.round((itemPriceTotal / sumPrice) * orderPayMoney * 100) / 100
                        : Math.round((orderPayMoney / Math.max(items.length, 1)) * 100) / 100;
                      const scoreShare = sumScorePrice > 0
                        ? Math.round((itemScoreTotal / sumScorePrice) * orderPayScore)
                        : Math.round(orderPayScore / Math.max(items.length, 1));

                      return (
                        <>
                          <span className="text-xl font-bold text-orange-600">
                            <span className="text-base text-orange-600 font-medium">¥{cashShare.toFixed(2)}</span>
                            <span className="text-base text-orange-600 font-medium mx-1">+</span>
                            <span className="text-base text-orange-600 font-mono font-bold">{scoreShare}</span>
                            <span className="text-base text-orange-600 font-medium ml-1">消费金</span>
                          </span>
                        </>
                      );
                    })() : (
                      <>
                        {order.pay_type === 'score' ? (
                          <>
                            <span className="text-xl font-bold text-orange-600">
                          ...
                          <>
                            <span className="text-xl font-bold text-orange-600">
                              ¥{(item.subtotal && Number(item.subtotal) > 0) ? formatAmount(item.subtotal) : formatAmount(item.price)}
                            </span>
                            <span className="text-xs text-orange-500 font-medium">
                              元
                            </span>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-baseline gap-1">
                {order.pay_type === 'mixed' && orderPayMoney > 0 && orderPayScore > 0 ? (
                  <>
                    <span className="text-xl font-bold text-orange-600">
                      <span className="text-base text-orange-600 font-medium">¥{orderPayMoney.toFixed(2)}</span>
                      <span className="text-base text-orange-600 font-medium mx-1">+</span>
                      <span className="text-base text-orange-600 font-mono font-bold">{orderPayScore}</span>
                      <span className="text-base text-orange-600 font-medium ml-1">消费金</span>
                    </span>
                  </>
                ) : order.pay_type === 'score' ? (
                  ...
                  <span className="text-xl font-bold text-orange-600">¥{formatAmount(order.total_amount)}</span>
                )}
              </div>
            </div>
          </div>
