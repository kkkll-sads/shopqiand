...
interface WithdrawOrderDetailProps {
    orderId: string;
    onBack: () => void;
}

const WithdrawOrderDetail: React.FC<WithdrawOrderDetailProps> = ({ orderId, onBack }) => {
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            try {
                // Since there is no dedicated detail API, we fetch the list and find the item
                // We try to fetch a larger list to increase chance of finding it
                // Ideally backend should support id filter or detail API
                ...
                if (isSuccess(res)) {
                    ...
                    if (found) {
                        ...
                        // If not found in first page, maybe we should show error
    ...
    return (
        <SubPageLayout title="提现详情" onBack={onBack}>
