        ...
        try {
            if (currentType === 'reset_login') {
                ...
                if (isSuccess(response)) {
                    ...
                    const errorMsg = (response && (response.msg || response.message)) ? (response.msg || response.message) : extractError(response, '修改密码失败');
                    setError(errorMsg);
                ...
                if (isSuccess(response)) {
                    ...
                    const errorMsg = (response && (response.msg || response.message)) ? (response.msg || response.message) : extractError(response, '修改支付密码失败');
                    setError(errorMsg);
                    showToast('error', '修改失败', errorMsg);
                }
            } else if (currentType === 'reset_pay_sms') {
                ...
                if (isSuccess(response)) {
                    ...
                    const errorMsg = (response && (response.msg || response.message)) ? (response.msg || response.message) : extractError(response, '重置支付密码失败');
                    setError(errorMsg);
